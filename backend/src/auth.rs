...
fn purpose_token(sub: &str, purpose: &str, ttl: i64, secret: &str)
    -> Result<String, jsonwebtoken::errors::Error>
{
    let exp = (chrono::Utc::now().timestamp() + ttl) as usize;
    encode(
        &Header::default(),
        &PurposeClaims { sub: sub.to_string(), exp, purpose: purpose.to_string() },
...
fn build_totp(secret_b32: &str, account: &str) -> Result<TOTP, &'static str> {
    ...
    TOTP::new(Algorithm::SHA1, 6, 1, 30, bytes, Some("Astraeusio".to_string()), account.to_string())
    ...
    match s.writer.create_user(email.clone(), hash).await {
        Ok(()) => {
            ...
            if let Some(ref mc) = s.mailer {
                if let Ok(token) = purpose_token(&email, "verify_email", 86_400, &s.jwt_secret) {
...
pub async fn login(State(s): State<AppState>, Json(body): Json<LoginRequest>) -> Response {
    ...
    if user.totp_enabled {
        match purpose_token(&user.email, "2fa_partial", 300, &s.jwt_secret) {
            Ok(partial) => return Json(serde_json::json!({ "requires_2fa": true, "partial_token": partial })).into_response(),
            Err(e) => {
...
pub async fn login_2fa(State(s): State<AppState>, Json(body): Json<TotpLoginRequest>) -> Response {
    ...
    let user = match s.db.lock().await.find_user_by_email(&email) {
        Ok(Some(u)) => u,
        Ok(None) => return (StatusCode::UNAUTHORIZED, Json(serde_json::json!({ "error": "user not found" }))).into_response(),
        Err(e) => {
    ...
    let Some(ref secret) = user.totp_secret else {
        return (StatusCode::BAD_REQUEST, Json(serde_json::json!({ "error": "2FA not configured" }))).into_response();
    };

    match check_totp(secret, &email, &body.code) {
        Ok(true)  => issue_jwt(&email, &s.jwt_secret),
        Ok(false) => (StatusCode::UNAUTHORIZED, Json(serde_json::json!({ "error": "invalid code" }))).into_response(),
...
pub async fn resend_verification(State(s): State<AppState>, claims: AuthClaims) -> Response {
    let Some(ref mc) = s.mailer else {
        return (StatusCode::SERVICE_UNAVAILABLE, Json(serde_json::json!({ "error": "email service not configured" }))).into_response();
    };

    let user = match s.db.lock().await.find_user_by_email(&claims.sub) {
        Ok(Some(u)) => u,
        _ => return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({ "error": "internal error" }))).into_response(),
    };

    if user.email_verified {
        return (StatusCode::CONFLICT, Json(serde_json::json!({ "error": "email already verified" }))).into_response();
    }
...
pub async fn setup_2fa(State(s): State<AppState>, claims: AuthClaims) -> Response {
    ...
    if user.totp_enabled {
        return (StatusCode::CONFLICT, Json(serde_json::json!({ "error": "2FA already enabled" }))).into_response();
    }
    ...
    let secret_b32 = match raw_secret.to_encoded() {
        Secret::Encoded(s) => s,
        _ => return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({ "error": "secret encoding failed" }))).into_response(),
    };
    ...
    if let Err(e) = s.writer.set_totp_secret(claims.sub, secret_b32.clone()).await {
...
pub async fn verify_2fa(
    ...
    let Some(ref secret) = user.totp_secret else {
        return (StatusCode::BAD_REQUEST, Json(serde_json::json!({ "error": "2FA setup not initiated" }))).into_response();
    };

    match check_totp(secret, &claims.sub, &body.code) {
        Ok(true) => {}
        Ok(false) => return (StatusCode::UNAUTHORIZED, Json(serde_json::json!({ "error": "invalid code" }))).into_response(),
...
fn issue_jwt(email: &str, secret: &str) -> Response {
    ...
    let claims = AuthClaims { sub: email.to_string(), exp, auth_type: AuthType::Jwt };
        ...
        if token.starts_with("ast_") {
            ...
            match sub_opt {
                Some(sub) => {
                    ...
                    return Ok(AuthClaims { sub, exp: usize::MAX, auth_type: AuthType::ApiKey });
                }
                None => return Err((StatusCode::UNAUTHORIZED, Json(serde_json::json!({ "error": "invalid API key" }))).into_response()),
            }
        }
