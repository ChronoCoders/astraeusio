      ...
      <div className="p-6 flex flex-col gap-5 flex-1">
        ...
        <div className="flex flex-col gap-2 mt-auto">
          {isFree ? (
            <button
              onClick={onCta}
              className="w-full py-2.5 rounded-lg text-sm font-medium transition-colors bg-zinc-800 hover:bg-zinc-700 text-zinc-100"
            >
              {t(`pricing.plans.${plan.key}.cta`)}
            </button>
          ) : (
            <a
              href={`mailto:altug@bytus.io?subject=Astraeusio ${plan.key.charAt(0).toUpperCase() + plan.key.slice(1)} Inquiry`}
              className={`w-full py-2.5 rounded-lg text-sm font-medium transition-colors text-center ${
                plan.highlight
